...
- Create a `text` file named `stockholm.txt`.
...
- Show the first 10 lines of stockholm.txt.

```bash
head stockholm.txt
```

- Show the first 5 lines of stockholm.txt.

```bash
head -5 stockholm.txt
```

- Show the last 10 lines of stockholm.txt.

```bash
tail stockholm.txt
```

- Show the last 5 lines of stockholm.txt.

```bash
tail -5 stockholm.txt
```

- Display the stockholm.txt file on the screen.
...
- View the stockholm.txt file with the `more` command.

```bash
more stockholm.txt
```

- View the stockholm.txt file with the `less` command.
...
- Display stockholm.txt file in reverse.

```bash
tac stockholm.txt
```

- Create reverse-stockholm.txt in reverse of stockholm.txt.

```bash
tac stockholm.txt > reverse-stockholm.txt
```
...
- Find all the files whose name is stockholm.txt in a current working directory.

```bash
find . -name stockholm.txt
```

- Find all the files under /home directory with name stockholm.txt.

```bash
find /home -name stockholm.txt
```

- Find all the files whose name is stockholm.txt and contains both capital and small letters in /home directory.

```bash
find /home -iname stockholm.txt
```
...
- Sometimes we need more content in search results to decide what is most relevant. For this, we can use the following operators to add the desired lines before, after a match, or both:
    ...
    ```bash
    grep -A3 "line 5" stockholm.txt # this command prints three lines after the match.
    ```

    - Use -B and a number of lines to display before a match.
    
    ```bash
    grep -B2 "line 5" stockholm.txt # this command prints two lines before the match.
    ```

    - Use -C and a number of lines to display before and after the match.
    
    ```bash
    grep -C4 "line 5" stockholm.txt # this command prints four lines before and after the match.
    ```
